...
{
    // 1. Redirect cout to our stringstream
    std::stringstream buffer;
    std::streambuf *sbuf = std::cout.rdbuf();
    std::cout.rdbuf(buffer.rdbuf());

    // 2. Vérifier si le output du main est équivalent
    std::string expected =
